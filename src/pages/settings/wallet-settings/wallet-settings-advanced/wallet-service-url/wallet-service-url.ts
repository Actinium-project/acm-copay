...
export class WalletServiceUrlPage {
  ...
  ionViewWillEnter() {
    ...
    this.comment = this.replaceParametersProvider.replace(
      this.translate.instant(
        "{{appName}} depends on Bitcore Wallet Service (BWS) for blockchain information, networking and Copayer synchronization. The default configuration points to https://bws.actinium.org (Actinium Project's public BWS instance)."
      ),
      { appName }
    );
  ...
  public save(): void {
    ...
    switch (this.walletServiceForm.value.bwsurl) {
      ...
      case 'loc':
      case 'local':
        bws = 'http://bws.actinium.org:3232/bws/api';
        break;
    }
    if (bws) {
