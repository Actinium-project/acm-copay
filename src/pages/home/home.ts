...
export class HomePage {
  ...
  private openEmailDisclaimer() {
    let message = this.translate.instant(
      'By providing your email address, you give explicit consent to The Actinium Core Team to use your email address to send you email notifications about payments.'
    );
    ...
    return this.clipboardProvider
      ...
      .then(async data => {
        ...
        if (this.validDataFromClipboard.type === 'PayPro') {
          const coin: string =
            data.indexOf('bitcoincash') === 0 ? Coin.BCH : Coin.ACM;
